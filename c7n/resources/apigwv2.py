...
@HttpApi.filter_registry.register('cross-account')
...
@query.sources.register('describe-http-stage')
class DescribeHttpStage(query.ChildDescribeSource):

    def get_query(self):
        query = super(DescribeHttpStage, self).get_query()
...
@resources.register('http-stage')
class HttpStage(query.ChildResourceManager):

    class resource_type(query.TypeInfo):
        service = 'apigateway'
        parent_spec = ('http-api', 'ApiId', None)
    ...
    def get_arns(self, resources):
        arns = []
        for r in resources:
            arns.append(self.generate_arn('/apis/' + r['ApiId'] +
             '/stages/' + r[self.get_model().id]))
        return arns


@HttpStage.action_registry.register('update')
...
@HttpStage.action_registry.register('delete')
class DeleteStage(BaseAction):
    ...
    def process(self, resources):
        client = utils.local_session(self.manager.session_factory).client('apigatewayv2')
        for r in resources:
            try:
                self.manager.retry(
                    client.delete_stage,
...
@HttpStage.filter_registry.register('client-certificate')
